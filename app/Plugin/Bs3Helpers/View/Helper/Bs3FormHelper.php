...
class Bs3FormHelper extends FormHelper {
    ...
    public function create($model = null, $options = array()) {
        ...
        $optionsForCreate = Hash::merge(
                        $this->formOptions, array('inputDefaults' => $this->inputOptions)
        );
    ...
    /*
     * Extends this->input to create an input together with a Last Year Response Tooltip.
     */

    public function inputLYRT($fieldName, $options = array()) {

        $outPutData = $this->input($fieldName, $options);

        if (isset($options['tooltip'])) {
            $outPutData .= $this->lastYearResponseToolTip($options['tooltip']);
        }
    ...
    /*
     * Clicking this tooltip will result in loading the response from last year to this question.
     */

    public function lastYearResponseToolTip($options) {

        $outputData = "";
        if (isset($options['target'])) {
            $tooltipTitle = isset($options['content']) ? $options['content'] : '';
            $associations = isset($options['associations']) ? $options['associations'] : array('keys' => array(), 'values' => array());
            $tooltipTitle = $this->buildToolTipFromJSON($tooltipTitle, $associations);
            $tooltipTitle = strpos($tooltipTitle, "''") ? $tooltipTitle : "''" . $tooltipTitle . "''";
            $tooltipTitle = 'L\'année dernière la réponse a été: ' . $tooltipTitle;
            $outputData = ' <span class="tip glyphicon glyphicon-plus-sign last-year-data-tooltip" title="' . $tooltipTitle . '" data-target="' . $options['target'] . '"></span>';
        }
        return $outputData;
    }

    private function buildToolTipFromJSON($tooltipTitle, $associations = array('keys' => array(), 'values' => array())) {
        $jsonTitle = json_decode($tooltipTitle, true);
        if (is_array($jsonTitle)) {
            $decodedTitle = "";
            foreach ($jsonTitle as $key => $elem) {
                $key = isset($associations['keys'][$key]) ? $associations['keys'][$key] : $key;
                $decodedTitle .= $key . ": ''" . $this->buildToolTipFromJSON($elem, $associations) . "'', ";
            }
            return substr($decodedTitle, 0, -2);
        } else {

            $tooltipTitle = isset($associations['values'][$tooltipTitle]) ? $associations['values'][$tooltipTitle] : $tooltipTitle;
            return $tooltipTitle;
        }
    }
    ...
    public function datepicker($fieldName, $options = array()) {
        ...
        $dp_required = isset($options['required']) ? $options['required'] : false;
        $dp_label = array(
            'width' => isset($options['label']['width']) ? $options['label']['width'] : 'col-md-2',
            'value' => isset($options['label']['text']) ? $options['label']['text'] : ''
        );
        $dp_width = isset($options['width']) ? $options['width'] : 'col-md-5';

        $additional_classes = isset($options['additionalClass']) ? $options['additionalClass'] : '';

        /* $dp_save_format = isset( $options['save_format'] )?$options['save_format']:'mm/dd/yyyy';
          $dp_display_format = isset( $options['display_format'] )?$options['display_format']:'mm/dd/yyyy'; */
        $dp_save_format = $dp_display_format = 'mm/dd/yyyy';

        $dp_value = isset($options['value']) ? $options['value'] : '';
        $dp_value = !empty($dp_value) && $dp_value != '0000-00-00' ? date('m/d/Y', strtotime($dp_value)) : '';

        $dp_link_id = $fieldName . "_dp_";

        $outputData .= $options['label'] ? '<label for="dtp_input2" class="' . ($dp_required ? ' required ' : '') . 'control-label ' . $dp_label['width'] . ' control-label">' . $dp_label['value'] . '</label>' : '';
        $outputData .= '<div class="input-group date form_date ' . $dp_width . ' ' . $additional_classes . '" data-date="' . $dp_value . '" data-date-format="' . $dp_display_format . '" data-link-field="' . $dp_link_id . '" data-link-format="' . $dp_save_format . '">'
                . '<input id="' . $dp_link_id . '" class="form-control show_calendar_input" size="16" type="text" value="' . $dp_value . '" readonly>';

        $linkOptions = array('required' => $dp_required, 'value' => $dp_value, 'type' => 'text', 'label' => false, 'div' => false, 'wrap' => false, 'class' => 'form-control show_calendar_input datepicker-value-field', 'size' => 16);

        $outputData .= ($this->input($fieldName, $linkOptions));

        $outputData .= '<span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                        <span class="input-group-addon show_calendar_input"><span class="glyphicon glyphicon-calendar"></span></span>
                    </div>';

        return $outputData;
        /* $outputData .= '<div class="input-group date form_date ' . $dp_width . '" data-date="' . $dp_value . '" data-date-format="' . $dp_display_format . '" data-link-field="' . $dp_link_id . '" data-link-format="' . $dp_save_format . '">
          <input class="form-control show_calendar_input" size="16" type="text" value="' . $dp_value . '" readonly>
          <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
          <span class="input-group-addon show_calendar_input"><span class="glyphicon glyphicon-calendar"></span></span>
          </div>';

          $linkOptions = array('id' => $dp_link_id, 'value'=>$dp_value, 'type' => 'hidden', 'required' => $dp_required, 'label' => false, 'div' => false, 'wrap' => false, 'class' => 'form-control');

          return $outputData . (parent::input($fieldName, $linkOptions)); */
    }
    ...
    public function radio($fieldName, $options = array(), $attributes = array()) {
        ...
        foreach ($options as $optValue => $optTitle) {
            ...
            $optHtml = $this->Html->useTag('radio', $attributes['name'], $tagName, array_diff_key($allOptions, array('name' => null, 'type' => null, 'id' => null)), ''
            );
    ...
    protected function _processConfig($options) {
        ...
        $this->inputOptions = Hash::merge(
                        Configure::check('Bs3.Form.inputDefaults') ? Configure::read('Bs3.Form.inputDefaults') : array(), $this->_processCustomConfig('input', $styleInputDefaults), $this->_processCustomConfig('input', $this->_extractOption('inputDefaults', $options, array()))
        );
        ...
        $this->formOptions = Hash::merge(
                        $this->_myFormDefaults, Configure::check('Bs3.Form.formDefaults') ? Configure::read('Bs3.Form.formDefaults') : array(), $this->_processCustomConfig('form', $styleFormDefaults), $this->_processCustomConfig('form', $options)
        );
